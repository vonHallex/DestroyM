...
void blockManager::swapBlocks(block* blockA, block* blockB,Vec2 pos /* =0 */)
{
	//int indexA = blockA->getIndex();
	//Vec2 posA = Vec2(indexA/6 *60, indexA%6 *60);
	blockB->runAction(MoveTo::create(0.5f,pos));

	blockStore.swap(blockA,blockB);	
}

  void blockManager::swapByPosition(Vec2 preLocation,Vec2 finLocation)
{
	block* blockA = getCurrentBlockByPos(preLocation);
	block* blockB = getCurrentBlockByPos(finLocation);
